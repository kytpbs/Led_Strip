...
void arrayFunctions::reverseValues(int* array) {
    reverseValues(array, sizeof(array) / sizeof(array[0]), 255);
}
...
uint32_t LedStrip::Color(CloudColoredLight* color) {
    ...
    white = (color->getSaturation() / 100.0) * (color->getBrightness() / 100.0) * 255;
...
void LedStrip::changeLedColor(CloudColoredLight* color) {
    ...
    uint8_t blue;
    color->getCloudValue().getRGB(red, green, blue);
    uint32_t color32 = Color(red, green, blue);
...
void LedStrip::breathe(unsigned long delayTime) {
    ...
    smoothChangeTo(this->color, delayTime); // turn back on
}
...
void LedStrip::setNightMode(bool active) {
    if (active) {
        ...
        Serial.println("Changing mode to Normal");
        changeMode(this->lastMode != Modes::Off ? this->lastMode : Modes::Normal);
    }
}
