...
void Fully_Saturate_With_Brightness(int* colors, int brightness) {
  ...
  increase_by = increase_by * (brightness / 100.0);
  if (colors[0] > 50) {
...
void HSBtoSaturatedRGB(int hue, int sat, int bright, int* colors) {
	H2R_HSBtoRGB(hue, sat, bright, colors);
	Fully_Saturate_With_Brightness(colors, bright);
}
